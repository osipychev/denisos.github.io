...
function update(){
    ...
    if (sim_state == sim_states.reset){
        ...
        for (var i = 0; i < n_agents; ++i){
            agent_list[i] = new FarmAgent(i,mdp_random_state(n_dim),1);
    ...
    if (sim_state == sim_states.run){
        for (var i = 0; i < n_agents; ++i){
            ...
            if (agent_list[i].mode == agent_list[i].modes.idle){
                console.log(agent_list[i].report());
                agent_list[i].resetReward();
                agent_list[i].goRight();
                agent_list[i].mode = agent_list[i].modes.scout;
            }
        }
    }
...
function draw(){
    ...
    if (document.getElementById("show_goal").checked)
        myGrid.show_colors(canvas,reward,n_dim);
    
    myGrid.show_state(canvas,agent_list,n_dim);
