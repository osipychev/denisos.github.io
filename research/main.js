...
function update(){
    ...
    if (sim_state == sim_states.reset){
        ...
        //mdp_init();
        weeds = new Weeds(document.getElementById("wscale").value/100);
    ...
    if (sim_state == sim_states.run){
        ...
        //update participants
        reward = weeds.step([n_dim,n_dim],dt);
        for (var i = 0; i < n_agents; i++){
...
function draw(){
    ...
    myEnv.show_colors(canvas,weeds.getReward());
    ...
    if (plotTimer < total_time){
        ...
        reward_hist.push({x:total_time/3600,y:-totalReward(reward)});
